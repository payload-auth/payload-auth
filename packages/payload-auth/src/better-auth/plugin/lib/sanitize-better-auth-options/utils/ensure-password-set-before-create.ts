 ...
 */
export function ensurePasswordSetBeforeUserCreate(
  options: SanitizedBetterAuthOptions
) {
  if (typeof options.databaseHooks !== "object") options.databaseHooks = {};
  if (typeof options.databaseHooks.user !== "object")
    options.databaseHooks.user = {};
  if (typeof options.databaseHooks.user.create !== "object")
    options.databaseHooks.user.create = {};
  const initialBeforeUserCreateHook =
    options.databaseHooks.user.create.before ?? null;
