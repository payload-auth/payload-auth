...
export const getAfterDeleteHook =
  (): CollectionAfterDeleteHookWithBetterAuth => {
    const hook: CollectionAfterDeleteHookWithBetterAuth = async ({
      doc,
      req,
    }) => {
      try {
        ...
        const userId = doc.id;
        const beforeDelete =
          betterAuthContext.options.user?.deleteUser?.beforeDelete;
        ...
        const afterDelete =
          betterAuthContext.options.user?.deleteUser?.afterDelete;
