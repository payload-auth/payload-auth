...
export function getAfterLogoutHook() {
  const hook: CollectionAfterLogoutHook = async ({ req }) => {
    ...
    const { sessionToken, sessionData, dontRememberToken } = (
      await payload.betterAuth.$context
    ).authCookies;
    ...
    await deleteSessionFromDb(
      payload,
      sessionsSlug,
      store.get(sessionToken.name)?.value,
      req
    );
    ...
    const multiCandidates = store
      ...
      .filter(
        (c) =>
          c.name.startsWith(multiBase) ||
          c.name.startsWith(`__Secure-${multiBase}`)
      )
      .map((c) => c.name);
