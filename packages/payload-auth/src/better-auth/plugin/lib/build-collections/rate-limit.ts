...
export function buildRateLimitCollection({
  incomingCollections,
  pluginOptions,
  resolvedSchemas
  ...
  const existingRateLimitCollection = incomingCollections.find(
  ...
  if (
    typeof pluginOptions.pluginCollectionOverrides?.rateLimit === "function"
  ) {
