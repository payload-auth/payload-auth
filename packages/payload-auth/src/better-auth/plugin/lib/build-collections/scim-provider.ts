...
export function buildScimProviderCollection({ incomingCollections, pluginOptions, resolvedSchemas }: BuildCollectionProps): CollectionConfig {
    ...
    const existingScimProviderCollection = incomingCollections.find((collection) => collection.slug === scimProviderSlug) as
        | CollectionConfig
        | undefined

    const fieldOverrides: FieldOverrides<keyof ScimProvider> = {
        providerId: () => ({
            admin: { readOnly: true, description: 'The provider ID. Used to identify a provider and to generate a redirect URL.' }
        }),
        scimToken: () => ({
            admin: { readOnly: true, description: 'The SCIM bearer token. Used by your identity provider to authenticate against your server.' }
        }),
        organizationId: () => ({
            admin: { readOnly: true, description: 'The organization Id. If provider is linked to an organization.' }
        })
    }
    ...
    let scimProviderCollection: CollectionConfig = {
        ...
        fields: [...(existingScimProviderCollection?.fields ?? []), ...(collectionFields ?? [])]
    }
