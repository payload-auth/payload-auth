...
export function buildOrganizationsCollection({ incomingCollections, pluginOptions, resolvedSchemas }: BuildCollectionProps): CollectionConfig {
  ...
  const joinFields: JoinField[] = [
    ...
    },
    {
      ...
      hasMany: true,
      collection: teamSlug,
      on: getSchemaFieldName(resolvedSchemas, baModelKey.team, baModelFieldKeys.team.organizationId),
      maxDepth: 1,
      saveToJWT: false
    },
    {
  ...
  let organizationCollection: CollectionConfig = {
    ...
    fields: [...(existingOrganizationCollection?.fields ?? []), ...(collectionFields ?? []), ...joinFields]
  }
