...
export function buildOrganizationRolesCollection({ incomingCollections, pluginOptions, resolvedSchemas }: BuildCollectionProps): CollectionConfig {
    const organizationRolesSlug = getSchemaCollectionSlug(resolvedSchemas, baModelKey.organizationRole)
    const organizationRolesSchema = resolvedSchemas[baModelKey.organizationRole]

    const existingOrganizationRolesCollection = incomingCollections.find((collection) => collection.slug === organizationRolesSlug) as
        | CollectionConfig
        | undefined
    ...
    let organizationRolesCollection: CollectionConfig = {
        ...
        admin: {
            ...
            useAsTitle: getSchemaFieldName(resolvedSchemas, baModelKey.organizationRole, 'role'),
            description: 'Organization roles are the roles that are available for an organization.',
        ...
        fields: [...(existingOrganizationRolesCollection?.fields ?? []), ...(collectionFields ?? [])]
    }

    if (typeof pluginOptions.pluginCollectionOverrides?.organizationRoles === 'function') {
        organizationRolesCollection = pluginOptions.pluginCollectionOverrides.organizationRoles({
