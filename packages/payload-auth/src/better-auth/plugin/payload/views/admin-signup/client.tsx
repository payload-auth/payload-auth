  ...
  return (
    <div>
      {requireEmailVerification ? (
        <h1 className={'email-verification-required'}>Please verify your email to login. Check your email for a verification link.</h1>
      ) : (
        <>
          <h1>{t('general:welcome')}</h1>
          <p>{t('authentication:beginCreateFirstUser')}</p>
          <Form
            action={`${serverURL}${apiRoute}/${userSlug}${adminEndpoints.signup}?token=${token}&role=${role}&redirect=${redirectUrl}`}
            method="POST"
