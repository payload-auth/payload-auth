...
export const createTransform = (
  ...
  function normalizeData({
      ...
      if (idType === "number" && typeof incomingValue === "string") {
        ...
        if (!isNaN(parsed)) {
          debugLog([
            ...
            { original: incomingValue, converted: parsed },
          ]);
          return parsed;
        }
        ...
        debugLog([
          ...
          { original: incomingValue, converted: stringId },
        ]);
        return stringId;
      }
      ...
      if (typeof incomingValue !== "string") {
        if (incomingValue instanceof Date) {
          ...
          debugLog([
            ...
            { original: incomingValue, converted },
          ]);
          return converted;
        }
    ...
    if (fieldType === "boolean") {
      if (typeof incomingValue !== "boolean") {
        ...
        if (typeof incomingValue === "string") {
          converted =
            incomingValue.toLowerCase() === "true" || incomingValue === "1";
    ...
    if (typeof fieldType === "string" && fieldType.endsWith("[]")) {
      ...
      if (!Array.isArray(incomingValue)) {
        ...
        debugLog([
          ...
          { original: incomingValue, converted: arrayValue },
        ]);
      }
          ...
          if (typeof item === "string") {
            ...
            if (!isNaN(parsed)) {
              debugLog([
                ...
                { original: item, converted: parsed },
              ]);
              return parsed;
            }
  ...
  function transformOutput<T extends Record<string, any> | null>({
    ...
    const relationshipFields = Object.entries(schemaFields)
      ...
      .reduce(
        ...
        {} as Record<string, FieldAttribute<FieldType>>
      );
  ...
  function findRelationshipOriginalKey(
    ...
    return Object.keys(relationshipFields).find(
      (key) => relationshipFields[key].fieldName === fieldName
    );
  }
  ...
  function convertSort(
    ...
    return `${sortBy.direction === "desc" ? "-" : ""}${getFieldName(
      model,
      sortBy.field
    )}`;
  }
