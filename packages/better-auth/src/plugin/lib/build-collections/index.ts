...
export function buildCollections({
  ...
  const buildCollectionMap = {
    [baseCollectionSlugs.users]: () =>
      buildUsersCollection({ incomingCollections, sanitizedBAOptions, pluginOptions }),
    [baseCollectionSlugs.accounts]: () =>
      buildAccountsCollection({ incomingCollections, pluginOptions }),
    [baseCollectionSlugs.sessions]: () =>
      buildSessionsCollection({ incomingCollections, sanitizedBAOptions, pluginOptions }),
  ...
  const collectionConfigs = Array.from(requiredCollectionSlugs)
    .map((slug) => buildCollectionMap[slug as keyof typeof buildCollectionMap]?.())
    .filter(Boolean)
