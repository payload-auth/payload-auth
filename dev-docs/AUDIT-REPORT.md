...
This audit identified **5 critical (P0)**, **12 high (P1)**, **20 medium (P2)**, and **13 low (P3)** issues across the plugin codebase. The most severe findings include:
...
### P0-3: Admin Invite Endpoints Have No Authentication
...
**Fix:** Add authentication and admin-role checks at the top of both handlers:
```typescript
if (!req.user) return Response.json({ message: "Unauthorized" }, { status: 401 });
if (!hasAdminRoles(adminRoles)({ req })) return Response.json({ message: "Forbidden" }, { status: 403 });
```

---

### P0-4: Invite Token Race Condition Enables Multi-User Role Escalation

**File:** `plugin/lib/build-collections/users/endpoints/set-admin-role.ts:39-72`
**Type:** Security - Race Condition

The `set-admin-role` endpoint performs three non-atomic operations:
...
**Fix:**
- Add an expiration field to admin invitations and check it
- Use an atomic find-and-delete operation (or a transaction)
- Make `token` required in the zod schema
- Validate the `redirect` parameter against a whitelist or ensure it's a relative URL
...
### P1-8: Admin Invite Role Not Applied for Social Provider Sign-Ups

**File:** `plugin/lib/sanitize-better-auth-options/utils/use-admin-invite-after-email-sign-up-middleware.ts:22`
**Type:** Logic Error

The **before** middleware (`requireAdminInviteForSignUpMiddleware`) validates invite tokens for both `/sign-up/email` and `/sign-in/social`. But the **after** middleware only processes `/sign-up/email`:
...
### P2-9: `originalAfter` Middleware Not Awaited
...
The same pattern appears in `use-admin-invite-after-email-sign-up-middleware.ts` (lines 23, 40, 46, 75). Async middleware results are fire-and-forget; errors become unhandled promise rejections.
...
### P2-13: `set-admin-role` Replaces Roles Instead of Merging

**File:** `plugin/lib/build-collections/users/endpoints/set-admin-role.ts:60`

```typescript
data: { role: [role] }
```

A user with `["user", "editor"]` who accepts an admin invite for `"admin"` loses their existing roles entirely.
...
### Dual-Session Inconsistency

The most fundamental challenge is the dual-session architecture. Payload uses JWT tokens while Better Auth uses database-backed sessions. The `afterLogin` and `afterLogout` hooks bridge these, but edge cases exist:

- Payload JWT expiry and Better Auth session expiry can differ
- Admin revoking a Better Auth session doesn't invalidate the Payload JWT
- `afterLogin` failure creates a JWT-only state with no Better Auth session
- No mechanism to revoke Payload JWTs when Better Auth sessions are revoked
...
3. Add authentication checks to invite endpoints
4. Add atomic token consumption with expiration to set-admin-role
...
13. Handle social sign-up path in admin invite after middleware
...
### Medium-Term (P2)
16. Implement proper transactions using Payload's database adapter
17. Abstract cookie operations to support non-Next.js environments
18. Add banned-user check in auth strategy
19. Use `crypto.timingSafeEqual` for password verification
20. Add role-guarding type to `normalizeData`
21. Apply consistent field name mapping across sort/select/where
22. Fix date/relationship field detection in transformOutput
23. Await all async middleware
24. Hide sensitive fields (2FA secrets, JWKS private keys) in admin UI
25. Add missing `references.model` configurations for SSO, API Key, OIDC plugins

### Long-Term (Architecture)
26. Consider a session synchronization mechanism to handle dual-session edge cases
